...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed read config file %s: %v", o.configPath, err)
	}
	var ssc secretsConfig
	if err := yaml.Unmarshal(content, &ssc); err != nil {
		log.Fatalf("Failed unmarshalling secrets config: %v", err)
	}
	...
	for _, sc := range ssc.Secrets {
		...
		if err != nil {
			log.Fatalf("Failed creating secret manager client: %v", err)
		}
	...
	if err != nil {
		log.Fatalf("Error getting infrastructure cluster config: %v", err)
	}
